...
hi_outflow1_smo = pd.DataFrame(columns=['Velocity_kms','Intensity_cm-2'])
hi_outflow2_smo = pd.DataFrame(columns=['Velocity_kms','Intensity_cm-2'])
for i in range(0,n_pointings):
	...
	hi_outflow1_smo = hi_outflow1_smo.append({'Velocity_kms': hi_vel_smo, 'Intensity_cm-2': hi_spec_1_smo_int}, ignore_index=True)
	hi_outflow2_smo = hi_outflow2_smo.append({'Velocity_kms': hi_vel_smo, 'Intensity_cm-2': hi_spec_2_smo_int}, ignore_index=True)
...
for i in range(0,n_pointings):
	...
	tab = {'Velocity_kms': hi_outflow1_smo.iloc[[i]]['Velocity_kms'].values[0], 'Intensity_cm-2': hi_outflow1_smo.iloc[[i]]['Intensity_cm-2'].values[0]}
	pd.DataFrame.from_dict(tab).to_csv('../Data/Outflow_Pointings/HI/outflow1_pix'+str(i)+'_smo.csv',index=False)
	tab = {'Velocity_kms': hi_outflow2_smo.iloc[[i]]['Velocity_kms'].values[0], 'Intensity_cm-2': hi_outflow2_smo.iloc[[i]]['Intensity_cm-2'].values[0]}

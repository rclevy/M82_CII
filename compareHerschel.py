...
pacs_peak,_,_ = pacs_Jy2K(pacs_peak_Jypix,pacs_peak_hdr) #Jy/beam

pacs_peak_levels = np.linspace(1.,5.,5)
...
sofia_peak,_ = sofia_K2Jy(sofia_peak[0].data,sofia_peak_hdr)
...
im=ax.imshow(sofia_peak,origin='lower',cmap=sofia_peak_cmap,vmin=sofia_peak_vmin,transform=ax.get_transform(sofia_peak_wcs))
con=ax.contour(pacs_peak,origin='lower',levels=pacs_peak_levels,cmap=pacs_peak_cmap,linewidths=2.5)
ax.set_xlim(xlim)
ax.set_ylim(ylim)
cb = plt.colorbar(im,shrink=0.7)
cb.add_lines(con)
cb.set_label('[CII] Peak Intensity (Jy beam$^{-1}$)')
...
for j in range(n_pixels):
	...
	cii_spec,_ = sofia_K2Jy(np.flip(cii_spec),sofia_peak_hdr)
	...
	pacs_spec,_,_ = pacs_Jy2K(pacs_spec_Jypix,pacs_peak_hdr)
	...
	c0=ax.fill_between(cii_vel,cii_spec,color=sofia_color,alpha=0.5,step='pre')
	c1,=ax.step(cii_vel,cii_spec,color=sofia_color,lw=1.5,label='SOFIA')
	c2,=ax.step(pacs_vel,pacs_spec,color=pacs_color,lw=1.75,label='PACS')

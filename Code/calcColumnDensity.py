...
def SigRuln_H2(T,n):
	Rul_H2 = 3.8E-10*(T/100)**0.14 #Goldsmith+2012, Eq 43
	return Rul_H2*n
...
def NH2_CII(NCp_from_TpCII):
	C_H = 1.5E-4 #C/H, Gerin+2015
	C_H2 = 2*C_H
	#assumes all C singly-ionized and all H molecular
	return NCp_from_TpCII/C_H2
...
for i in range(n_pixels):
	...
	Vclip_red,Vclip_blue = upGREATUtils.get_bad_edge_chans(0,pix_order_labels[i])
	print(Vclip_red)
	...
	#calculate the CII column density for collisions with only H2
	N_cii_H2 = NCp_from_TpCII(cii_spec,dv,B_T(T,SigRuln_H2(T,n_CNM))) #cm^-2
	N_h2_cii = NH2_CII(N_cii_H2)
	...
	#get uncerts from a Monte Carlo of T and n
	ntrials = 500
	T_trials = np.random.uniform(low=T-eT,high=T+eT,size=(1,ntrials))
	n_trials = np.random.uniform(low=n_CNM-en_CNM,high=n_CNM+en_CNM,size=(1,ntrials))
	N_cii_trials = NCp_from_TpCII(cii_spec.reshape((len(cii_spec),1)),dv,B_T(T_trials,SigRuln(T_trials,n_trials)))#cm^-2
	N_hi_cii_trials = NHI_CII(N_cii_trials,f_CNM) #cm^-2
	N_h2_cii_trials = NH2_CII(NCp_from_TpCII(cii_spec.reshape((len(cii_spec),1)),dv,B_T(T_trials,SigRuln(T_trials,n_trials)))) #cm^-2
	eN_cii = np.nanstd(N_cii_trials,axis=1)
	eN_hi_cii = np.nanstd(N_hi_cii_trials,axis=1)
	eN_h2_cii = np.nanstd(N_h2_cii_trials,axis=1)

	#sum the CII column
	N_cii_sum = np.nansum(N_cii)
	eN_cii_sum = np.sqrt(np.nansum(eN_cii**2))
	...
	this_eM_cii = MCp_from_NCp(eN_cii_sum,A_pix_cm2) #Msun
	...
	eMm_cii.append(this_eM_cii) #Msun
	eu_Mm_cii.append(this_M_cii_max-this_M_cii)
	el_Mm_cii.append(this_M_cii-this_M_cii_min)

	#get the high T and n limit
	N_hi_cii_lim = NHI_CII(NCp_from_TpCII(cii_spec,dv,B_T(np.inf,SigRuln(np.inf,np.inf))),f_CNM)
	...
	eratio_N_hi_cii = ratio_N_hi_cii*np.sqrt((eN_hi_cii/N_hi_cii)**2)
	...
	med_eratio_N_hi_cii = np.nanmedian(eratio_N_hi_cii[vix])
	med_ratios.append(med_ratio_N_hi_cii)
	med_eratios.append(med_eratio_N_hi_cii)
	...
	ixp = np.nanargmax(tt)
	peak_ratio = np.nanmax(N_hi*f_CNM)/N_hi_cii[ixp]
	peak_eratio = peak_ratio*np.sqrt((eN_hi_cii[ixp]/N_hi_cii[ixp])**2)
	peak_ratios.append(peak_ratio)
	peak_eratios.append(peak_eratio)
	...
	#plot the data for collisions with H0
	ax1 = fig1.add_subplot(gs1[this_row,start_cols[i]:start_cols[i]+2],)
	c2=ax1.fill_between(cii_vel[vix],N_hi[vix]/dv/norm,color=hi_color,alpha=0.1,step='pre')
	c0=ax1.fill_between(cii_vel[vix],N_hi_cii[vix]/dv/norm,color=cii_color,alpha=0.5,step='pre')
	c3,=ax1.step(cii_vel,N_hi/dv/norm,color=hi_color,lw=1.75,label='$N_{\mathrm{HI}}$')#\\%.1e' %(1/hi_sf))
	c6,=ax1.step(cii_vel,N_hi/dv/norm*f_CNM,color=hi_color,lw=1.,linestyle='-',label='$N_{\mathrm{CNM}}^{\mathrm{HI}}$ $_{(f_{\mathrm{CNM}}=%.1f)}$' %f_CNM)#\\%.1e' %(1/hi_sf))
	c1,=ax1.step(cii_vel,N_hi_cii/dv/norm,color=cii_color,lw=1.5,label='$N_{\mathrm{CNM}}^{\mathrm{[CII]}}$')

	#add high T and n limits
	...
	ax1.text(0.05,0.95,pix_order_labels[i],ha='center',va='top',transform=ax1.transAxes)
	ax1.text(0.975,0.975,'$\mathrm{\\frac{max(N_{CNM}^{HI})}{max(N_{CNM}^{[CII]})}=%.1f\pm%.1f}$' %(peak_ratio,peak_eratio),
		ha='right',va='top',fontsize=plt.rcParams['font.size']-4,transform=ax1.transAxes)

	# if i == 1:
	# 	leg = plt.legend([(c0,c1),(c2,c3),c6],[c1.get_label(),c3.get_label(),c6.get_label()],
	...
	if i == 1:
		leg = ax1.legend([(c2,c3),c6,(c0,c1)],[c3.get_label(),c6.get_label(),c1.get_label()],
	...
	#XCO = 1E20 #cm-2/(K km/s), somewhere in between
	# XCO_L = 0.7E20 #Leroy+2015
	# XCO_Y = 0.2E20 #Yuan+2022
	#XCO = 5*XCO
	N_h2_co = XCO*co_spec*dv #cm^-2
	# N_h2_co_L = XCO_L*co_spec*dv #cm^-2
	# N_h2_co_Y = XCO_Y*co_spec*dv #cm^-2
	...
	tt[cii_vel > 300] = np.nan
	ixp = np.nanargmax(tt)
	peak_ratio_CO = np.nanmax(N_h2_co)/N_h2_cii[ixp]
	peak_eratio_CO = peak_ratio_CO*np.sqrt((eN_h2_cii[ixp]/N_h2_cii[ixp])**2)
	peak_ratios_co.append(peak_ratio_CO)
	peak_eratios_co.append(peak_eratio_CO)
	# print('Pixel %i' %i)
	# print('   NH2_CO/N_H2_CII = %.2f' %peak_ratio_CO)

	#norm = 1E19


	#plot the data for collisions with H2
	ax2 = fig2.add_subplot(gs2[this_row,start_cols[i]:start_cols[i]+2],)
	c7=ax2.fill_between(cii_vel[vix],N_h2_cii[vix]/dv/norm,color=cii_color,alpha=0.5,step='pre')
	c8,=ax2.step(cii_vel,N_h2_cii/dv/norm,color=cii_color,lw=1.5,label='$N_{\mathrm{H_{2}}}^{\mathrm{[CII]}}$')


	#ax = plt.subplot(gs[this_row,start_cols[i]:start_cols[i]+2],)
	c4=ax2.fill_between(cii_vel[vix],N_h2_co[vix]/dv/norm,color=co_color,alpha=0.1,step='pre')
	c5,=ax2.step(cii_vel,N_h2_co/dv/norm,color=co_color,lw=1.75,label='$N_{\mathrm{H_{2}}}^{\mathrm{CO}}$')#\\%.1e' %(1/hi_sf))


	ax2.text(0.05,0.95,pix_order_labels[i],ha='center',va='top',transform=ax2.transAxes)
	ax2.text(0.975,0.975,'$\mathrm{\\frac{max(N_{H_{2}}^{CO})}{max(N_{H_{2}}^{[CII]})}=%.2f\pm%.2f}$' %(peak_ratio_CO,peak_eratio_CO),
		ha='right',va='top',fontsize=plt.rcParams['font.size']-4,transform=ax2.transAxes)
	...
	if i==5:
		ax2.set_xlabel('V$_{\mathrm{LSRK}}$ (km s$^{-1}$)')
		#ax.set_ylabel('N$_{\mathrm{HI}}$ ($\\times10^{21}$ cm$^{-2}$)')
	elif (i==0):
		ax2.set_ylabel('$N\\times10^{%i}$\n(cm$^{-2}$ (km s$^{-1}$)$^{-1}$)' %np.log10(norm))
	elif (i==1) | (i==3) | (i==4) | (i==6):
		#ax.xaxis.set_ticklabels([])
		ax2.yaxis.set_ticklabels([])

	if i == 1:
		leg = ax2.legend([(c7,c8),(c4,c5)],[c8.get_label(),c5.get_label()],
			handlelength=1.25,fontsize=plt.rcParams['font.size']-2,loc='center left',bbox_to_anchor=(1.025,0.625,0.2,0.2))



	#calculate the total column densities and masses
	N_hi_tot = np.nansum(N_hi)
	N_cnm_cii_tot = np.nansum(N_hi_cii)
	N_h2_co_tot = np.nansum(N_h2_co)
	m_H = 1.67E-24 #g
	Msun = 1.989E33 #g
	A_pix = np.pi*(14.1/2/206265*3.6E6*3.086E18)**2 #cm^2
	M_hi_tot = N_hi_tot*m_H*A_pix/Msun #Msun
	M_cnm_cii_tot = N_cnm_cii_tot*m_H*A_pix/Msun #Msun
	M_h2_co_tot = N_h2_co_tot*2*m_H*A_pix/Msun #Msun

	# print('   NHI_tot = %.2e, MHI_tot = %.2e' %(N_hi_tot,M_hi_tot))
	# print('   NCNM_CII_tot = %.2e, MCNM_CII_tot = %.2e' %(N_cnm_cii_tot,M_cnm_cii_tot))
	# print('   NH2_CO_tot = %.2e, MH2_CO_tot = %.2e' %(N_h2_co_tot,M_h2_co_tot))

#plt.savefig('../Plots/Outflow_Spectra/M82_ColumnDensity_CII_HI_Outflow1.pdf',bbox_inches='tight',metadata={'Creator':this_script})
fig1.savefig('../Plots/Outflow_Spectra/M82_ColumnDensity_CII_HI_Outflow1.pdf',bbox_inches='tight',metadata={'Creator':this_script})
fig2.savefig('../Plots/Outflow_Spectra/M82_ColumnDensity_CII_CO_Outflow1.pdf',bbox_inches='tight',metadata={'Creator':this_script})


Mmm_cii = Mm_cii.copy()
eMmm_cii = eMm_cii.copy()
...
tab = {'Pixel Number':pix_order_labels, 'ColumnDensity_Cp_cm2': Nn_cii,
	...
	'med_NHI_NHICII': med_ratios, 'peak_NHI_NHICII': peak_ratios, 'peak_NH2_NH2CII': peak_ratios_co}
# tab = {'Pixel Number':pix_order_labels, 'ColumnDensity_Cp_cm2': Nn_cii,
# 	'Mass_Cp_Msun':Mm_cii,'e_Mass_Cp_Msun':eMm_cii,
# 	'med_NHI_NHICII': med_ratios, 'peak_NHI_NHICII': peak_ratios, 'e_peak_NHI_NHICII': peak_eratios, 'peak_NH2_NH2CII': peak_ratios_co, 'e_peak_NH2_NH2CII': peak_eratios_co}
...
# 	tt[cii_vel > 300] = np.nan
# 	ixp = np.nanargmax(tt)
# 	peak_ratio_CO = np.nanmax(N_h2_co)/N_hi_cii[ixp]
# 	print('Pixel %i' %i)
# 	print(peak_ratio_CO)
...
# 	if i == 1:
# 		leg = plt.legend([(c2,c3),c6,(c0,c1),(c4,c5)],[c3.get_label(),c6.get_label(),c1.get_label(),c5.get_label()],
# 			handlelength=1.25,fontsize=plt.rcParams['font.size']-2,loc='center left',bbox_to_anchor=(1.025,0.625,0.2,0.2))

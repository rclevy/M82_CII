...
#also grab the CII spectra from the PACS cube
...
for i in range(0,n_pointings):
	...
	for j in range(pacs_cube.shape[0]):
		pacs_spec_1[j] = np.nansum(pacs_mask_1.multiply(pacs_cube[j,:,:],fill_value=np.nan))
		pacs_spec_2[j] = np.nansum(pacs_mask_2.multiply(pacs_cube[j,:,:],fill_value=np.nan))
	...
	for j in range(co_cube.shape[0]):
		co_tp_spec_1[j] = np.nansum(co_tp_mask_1.multiply(co_tp_cube[j,:,:],fill_value=np.nan))
		co_tp_spec_2[j] = np.nansum(co_tp_mask_2.multiply(co_tp_cube[j,:,:],fill_value=np.nan))

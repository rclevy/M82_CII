...
for i in range(n_pointings):
	...
	for orient in ['detector','sky']:
		...
		for j in range(n_pixels):
			...
			this_spec,this_vel,header,_ = load_upgreat_data('../Data/Outflow_Pointings/outflow'+str(i+1)+'_'+str(this_pix)+'.fits')
			...
			_,dist_pc = calc_pix_distance_from_gal_center(gal_center, header, distance)

			#plot the data
			ax.step(new_vel,new_spec,color='k')
			ax.text(0.05,0.95,this_pix_label,ha='left',va='top',transform=ax.transAxes)
